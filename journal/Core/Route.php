...
{ 
 ...
 $pathArray = []; 
 //Нам нужно, чтобы будущий роутинг считал, что мы находимся "в корне", 
а не вложены в каталог journal 
 ...
 //$_SERVER['REQUEST_URI'] возвращает URI, который был 
...
//Обязательно обернуть в array_values для сброса индексации. 
 ...
 //Обращаемся к методу нашего класса и получаем массив с 
роутингом 
 ...
 // подцепляем файл с классом модели (файла модели может и не 
быть) 
 ...
 */ 
 ...
 //Если у класса существует указанный выше метод action_index 
например, вызываем действие контроллера 
